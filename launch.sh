...
CUSTOM_ROOT_DRIVE="set-this-to-your-drive"
...
SD_ROOT_PATH="/media/$USER/$CUSTOM_ROOT_DRIVE/$AUTOMATIC1111_WEBUI"

export SD_ROOT_PATH="/media/$USER/$CUSTOM_ROOT_DRIVE/$AUTOMATIC1111_WEBUI"
...
#export COMMANDLINE_ARGS="--port 4433 --xformers --theme=dark --precision autocast --api"

#export COMMANDLINE_ARGS="--port 4433 --opt-split-attention --theme=dark --precision autocast --api --ckpt-dir "/home/$USER/SD/$AUTOMATIC1111_WEBUI/models" --embeddings-dir "/home/$USER/SD/$AUTOMATIC1111_WEBUI/models/embeddings" --medvram"

##########################################

echo "loading models from custom path:"

export COMMANDLINE_ARGS="--port 4433 --opt-split-attention --theme=dark --precision autocast --api --gfpgan-dir "$SD_ROOT_PATH/models/GFPGAN" --vae-dir "$SD_ROOT_PATH/models/VAE" --ckpt-dir "$SD_ROOT_PATH/models/" --embeddings-dir "$SD_ROOT_PATH/models/embeddings" --codeformer-models-path "$SD_ROOT_PATH/models/Codeformer" --gfpgan-models-path "$SD_ROOT_PATH/models/GFPGAN" --esrgan-models-path "$SD_ROOT_PATH/models/ESRGAN" --bsrgan-models-path "$SD_ROOT_PATH/models/ESRGAN" --realesrgan-models-path "$SD_ROOT_PATH/models/RealESRGAN" --medvram"
...
then
    ...
    printf "\n%s\n" "${delimiter}"
    "${GIT}" clone https://github.com/Randy420Marsh/stable-diffusion-webui.git "${clone_dir}"
    cd "${clone_dir}"/ || { printf "\e[1m\e[31mERROR: Can't cd to %s/%s/, aborting...\e[0m" "${install_dir}" "${clone_dir}"; exit 1; }
fi
